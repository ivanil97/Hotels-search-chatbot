...
from states.request_states import UserRequestStateLow
...
def low(message: Message) -> None:
    """
    Хендлер для обработки команды /low
    :param message: (Message) сообщение пользователя в чат-боте
    :return: None
    """
    # устанавливаем состояние пользователя
    bot.set_state(message.from_user.id, UserRequestStateLow.place, message.chat.id)

    # запрашиваем локацию
    bot.send_message(message.from_user.id, 'Где ищем?', reply_markup=ReplyKeyboardRemove())


@bot.message_handler(func=lambda message: '/' not in message.text, state=UserRequestStateLow.place)
def check_city(message: Message) -> None:
    """
    Хендлер для уточнения локации у пользователя (в рамках команды /low)
    :param message: (Message) сообщение пользователя в чат-боте
    :return: None
    """
...
@bot.callback_query_handler(
    func=lambda call: call.data not in ['None', 'history'] and ';' not in call.data, state=UserRequestStateLow.place)
def get_product(call: CallbackQuery) -> None:
    """
    Хендлер для сохранения названия локации и id по локации, указанной пользователем
    и запроса количества вариантов (в рамках команды /low)
    ...
    if call.data == 'new_request':
        bot.send_message(call.from_user.id, 'Создайте новый запрос /low')

    else:
        # устанавливаем состояние пользователя
        bot.set_state(call.from_user.id, UserRequestStateLow.quantity)
...
@bot.message_handler(state=UserRequestStateLow.quantity)
def get_quantity(message: Message) -> None:
    """
    Хендлер для сохранения количества вариантов и для подтверждения запроса у пользователя (в рамках команды /low)
    :param message: (Message) сообщение пользователя в чат-боте
    :return: None
    """
    ...
    if message.text.isdigit() and int(message.text) > 0:
        ...
        # устанавливаем состояние пользователя
        bot.set_state(message.from_user.id, UserRequestStateLow.options, message.chat.id)
...
@bot.message_handler(state=UserRequestStateLow.options)
def get_hotels_list(message: Message) -> None:
    """
    Хендлер для поиска отелей по id локации и количеству вариантов
    и сохранения запроса пользователя в БД (в рамках команды /low)
    :param message: (Message) сообщение пользователя в чат-боте
    :return: None
    """
    ...
    if message.text.lower() == 'да':
        ...
        # устанавливаем состояние пользователя
        bot.set_state(message.from_user.id, UserRequestStateLow.details, message.chat.id)
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            if not isinstance(response, str):
                ...
                create_db_data(user_name=message.from_user.username, user_id=message.from_user.id,
                               request_type='/low', request_body=short_request, hotels=hotels_str)
...
@bot.callback_query_handler(func=lambda call: call.data != 'None', state=UserRequestStateLow.details)
def get_hotel_details(call: CallbackQuery) -> None:
    """
    Хендлер для направления пользователю найденных вариантов по его запросу
    и переключения между ними (в рамках команды /low)
    :param call: CallbackQuery от пользователя в чат-боте
    :return: None
    """
